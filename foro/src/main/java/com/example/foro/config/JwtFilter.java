...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (this.tokenProvider.isValidToken(token)) {
            String email = this.tokenProvider.getUsernameFromToken(token); // Cambiado de getUsernameFromToken a getEmailFromToken

            // UserDetails representa al usuario
            UserDetails user = this.userDetailsService.loadUserByUsername(email); // Carga el usuario usando el email
